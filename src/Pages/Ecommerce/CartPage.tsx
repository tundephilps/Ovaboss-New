...
import { Cart, CheckoutItems } from "../../types/cart.type";
...
export default function ShoppingCart() {
	...
	const [carts, setCarts] = useState(cartItems as CheckoutItems[]);
	...
	const getTotalPrice = () => {
		return numberFormat(carts.reduce((total, item) => total + +item.price * (+item.quantity || 1), 0), 2);
	};

	const getSelectedTotalPrice = () => {
		return numberFormat(carts
			.filter((item) => selectedItems[item.productId])
			.reduce((total, item) => total + +item.price * (+item.quantity || 1), 0), 2);
	};
	...
	React.useEffect(() => {
		setCarts(cartItems as CheckoutItems[]);
		setCheckoutItems(cartItems as CheckoutItems[]);
	}, [cartItems])
			...
			<div className="grid lg:grid-cols-5 gap-12 grid-cols-1 lg:px-12 px-2 w-full mx-auto pb-12">
				...
				<div className="lg:col-span-3 col-span-5 p-4 shadow-md bg-white rounded">
					...
					{carts.map((item, key) => (
						<div
							key={key}
							className={`mb-6 pb-6 border-b ${selectedItems[item.productId] ? "bg-yellow-50" : ""
								}`}
						>
							<div className="flex flex-col sm:flex-row lg:space-y-0 space-y-4">
								...
								<div className="flex-1">
									...
									<button
										className={`flex items-center text-red-600 font-semibold mt-4 transition-opacity ${isSaving ? 'opacity-50 cursor-not-allowed' : ''
											}`}
					...
					<div className="p-4 rounded">
						...
						<div className="flex flex-col sm:flex-row gap-2 w-full">
							...
							<button
								...
								className={`${getSelectedCount() > 0 || carts.length > 0
									? "bg-yellow-500"
									: "bg-gray-300"
